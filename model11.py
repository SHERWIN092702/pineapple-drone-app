...
while True:
    ...
    saturation_multiplier = 1.3  # Safe boost between 1.2 to 1.5
    hsv_float[:, :, 1] *= saturation_multiplier

    # Reduce boost in very bright areas (optional safety)
    brightness_limit = 220
    hsv_float[:, :, 1][hsv_float[:, :, 2] > brightness_limit] *= 0.9

    # Clip saturation values to valid range
    ...
    for box in results[0].boxes:
        ...
        # Resize cropped pineapple (optional speed boost)
        crop_small = cv.resize(cropped_pineapple, (64, 64))

        # Convert to HSV and calculate average hue
